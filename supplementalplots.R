#probe statistics
summary_data <- tibble(Source = c("CHEMBL", "chemicalprobes.org", "Total"), `Targets` = c(367, 117, 444), `Probes` = c(717, 110, 827))
probe_list %>% dplyr::select(DATA_SOURCE, CHEMICAL_ID) %>% group_by(DATA_SOURCE)  %>% summarize(probes = n_distinct(CHEMICAL_ID))
length(unique(probe_list$CHEMICAL_ID))
probe_list %>% dplyr::select(DATA_SOURCE, TARGET_NAME) %>% group_by(DATA_SOURCE)  %>% summarize(TARGETS = n_distinct(TARGET_NAME))
length(unique(probe_list$pref_name))
probe_list %>% dplyr::filter(is_agonist == "YES") %>% count(is_agonist)
probe_list %>% count(orthogonal)
...
analysis_table <- tibble(Source = as.character(c("GO MF Level 3", "GO MF Level 4", "KEGG")), 
                         ...
                         `Least covered` = as.character(c(
                                                        ...
                                                        Melanogenesis")))
analysis_table$`Most covered` <- stringr::str_wrap(analysis_table$`Most covered`, width = 40)
analysis_table$`Least covered` <- stringr::str_wrap(analysis_table$`Least covered`, width = 40)
...
#I WILL MAKE THE TABLE MANUALLY
...
ggsave("totalactivitiesplot.png")
...
#targets found
cat(unique(probe_list$pref_name[grepl("hsa04014", probe_list$KEGG)]), sep = "; ")
#36
length(unique(probe_list$pref_name[grepl("hsa04014", probe_list$KEGG)]))
#21

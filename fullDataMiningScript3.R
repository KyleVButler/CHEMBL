...
# group the different types of activity measurements
selectivity_types <- c("Selectivity ratio", "Residual Activity", "Ratio IC50", "Ratio", "Ratio Ki", "Ratio EC50", "Fold selectivity", 
                              "Selectivity Index", "Selectivity index", "Residual activity",
                              "Relative potency", "Ratio pIC50", "Ratio pKi", "Selectivity", "Delta Tm", "Thermal melting change")
...
#keep only compounds that have at least one ic50 below 100 nM and collect data into R variable:
...
#remove some known pains structures (listed in text) and compounds i have found to be wrongly classified like 218012
#removed moracins (hif)
...
potency_measurements <- activities_collected %>% group_by(molregno) %>% 
  filter(target_type == "SINGLE PROTEIN" & organism == "Homo sapiens" & assay_type == "B" & confidence_score > 3 & 
...
activities_collected2 <- activities_collected %>% filter(molregno %in% compound_summary$molregno) %>% 
  filter(substr(pref_name, 1, 7) != "Cytochr") 
...
top_probes_onetarget <- compound_summary %>% group_by(pref_name, is_agonist, orthogonal) %>% 
  arrange(desc(n_select), desc(n_total)) %>% slice(1)
...
#keep only compounds with n_total > 4 and n_select > 3
compound_summary <- compound_summary %>% mutate(n_select = n_selectivity + n_potency) %>% select(molregno, n_total, n_select) %>% 
  filter(n_total > 4 & n_select > 3)

# make activities list with selected compounds and remove cytochrome data
...
cmpdstokeep <- activities_collected2 %>% 
  ...
  filter(assay_type == "B" & organism == "Homo sapiens" & standard_value > 1 &
           standard_value < 30 & accession == target_accession) %>%
  count(molregno) %>% filter(n > 2)  #number is 2 because both accessions will be included in measurement
compound_summary <- compound_summary %>% filter(!(molregno %in% unique(cmpdstokeep$molregno)))
...
#as a selectivity constraint, the compound must have measured potency in dose-response assays for 4 separate proteins,
#two of which are the main targets. The compound must be selective against two other proteins. 
...
compound_summary <- compound_summary %>% filter(molregno %in% unique(min_target$molregno))
activities_collected2 <- activities_collected2 %>% filter(molregno %in% unique(compound_summary$molregno))
...
#select probes with highest selectivity observations

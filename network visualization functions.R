...
reactome_probe_network <- function(pathway_name){
  probe_list <- suppressMessages(read_csv("PROBELIST.csv"))
  ...
  dev.new()
  viewPathway(pathway_name, organism = "human", fixed = TRUE, readable = TRUE, 
...
kegg_probe_network <- function(pathway_name){
  probe_list <- suppressMessages(read_csv("PROBELIST.csv"))
  gene_names <- as.numeric(unique(probe_list$ENTREZ_GENE))
  gene_names <- as.character(na.omit(gene_names))
  #keggview.native(gene.data = gene_names, res = 300, pathway.name = pathway_name, species = "hsa", col.key = FALSE)
  pathview(gene.data = gene_names, same.layer = TRUE, pathway.id = pathway_name, species = "hsa", col.key = FALSE)
...
highlight_nodes_cytoscape <- function(border_width = 6){
  print("Node names must be UNIPROT or ENTREZ GENE ids")
  print("Only one cytoscape window should be open")
  readline("Press enter to continue")
  probe_list <- suppressMessages(read_csv("PROBELIST.csv"))
  identifiers <- c(probe_list$UNIPROTKB, probe_list$ENTREZ_GENE)
...
plot_probe_target_network <- function(target, cutoff){
  ...
  for(i in 1:nrow(relevant_nodes)){
    if(relevant_nodes$node_type[i] != "middle")
    {
      relevant_nodes$hoverinfo[i] <- paste(relevant_nodes$alias[i], "<br>Chemical probes:<br>", 
                                           (probe_list[probe_list$ENTRY_NAME == relevant_nodes$alias[i], 1]))
    }
  }
  ...
  network <- plot_ly(data, type = "scatter", x = ~X, y = ~Y, mode = "markers", marker = list(size = 10), 
